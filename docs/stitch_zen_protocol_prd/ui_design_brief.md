...
AI-first single-user task management and capture system. Personal productivity tool with dark mode default, mobile-responsive layout, and AI-created content indicators.
...
│ ├─────────────────────────────────────────────┤ │
│ │ Bottom Nav (fixed)                          │ │
│ │ [Dashboard] [Tasks] [Captures] [Kanban]     │ │
│ └─────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────┘
```
...
- **Navigation**: Dashboard, Tasks, Captures, Kanban, Search, Settings (lucide icons)
...
7 card blocks, each with:
- Header: icon + title + count badge + "View all" link
- Content: list of TaskRow/CaptureRow items, or empty state message
...
### Captures (`/captures`)
- **List panel** with tab filters: Recent | Pinned | Reviewed | Archived (with count badges)
- **Detail panel** (right, desktop) or separate page (mobile)
- Promote action converts capture to task via PromoteDialog
...
### Search (`/search`)
- Global search across tasks and captures
...
### Capture
```
id, title, body, status (Captured|Reviewed|Archived),
pinned (boolean), context (Home|Work|null),
source (web_ui|mcp), nudgeAt (reminder date),
createdBy, agentId, reason, tags[]
```
...
- **CaptureRow**: Pin icon + Title + Body snippet + Status badge + Context + Tags (first 2) + AI badge
...
- Examples: "No tasks found", "No captures", "Drop tasks here"
